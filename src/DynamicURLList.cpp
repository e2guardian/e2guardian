...
{
    if (items == 0) {
        DEBUG_debug("url list cache is empty");
...
{
    DEBUG_debug("url cache search request: ", fg, " ", url);
    if (items == 0) {
        return false;
    }
#ifdef DEBUG_LOW
    DEBUG_debug("****** url cache table ******");
    ...
    if (pos > -1) {
        ...
        if ((timenow - urlreftime[index[pos]]) > timeout) {
            DEBUG_debug("found but url ttl exceeded: ", (timenow - urlreftime[index[pos]]));
            return false;
        }
        ...
        if (groups[index[pos]].find_first_of(lookfor) == std::string::npos) {
            ...
            DEBUG_debug("found but url not flagged clean for this group: ", fg, " (is clean for: ", ss.str(),  ")" );
#endif
            return false;
        }
        return true;
...
{
    DEBUG_debug("url cache add request: ", fg, " ", url, " itemsbeforeadd: ", items);
    int len = strlen(url);
    bool resized = false;
    char *u;
    ...
    if (pos >= 0) { // found
        ...
        DEBUG_debug("Entry found at pos: ", pos);
    ...
    DEBUG_debug("insertion pos: ", pos, " size: ", size);

    // the list isn't full, so simply push new entry onto the back
    if (items < size) {
        DEBUG_debug("items<size: ", items, "<", size);
        char *urlref;
