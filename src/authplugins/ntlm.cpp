        ...
        if (strcmp("NTLMSSP", a->h.signature) == 0 && WSWAP(a->h.type) == 3) {
            ...
            if ((l > 0) && (b >= 0) && (b + l) <= sizeof(a->payload) && (l <= 254)) {
                ...
                if (f & WSWAP(0x0001)) {
                    ...
                    if (ic == (iconv_t)-1) {
                        E2LOGGER_error("NTLM - Cannot initialise conversion from UTF-16LE to UTF-8: ", strerror(errno));
                        iconv_close(ic);
                        return -2;
                    }
                    size_t l2 = 256;
...
int ntlminstance::init(void *args)
{
    ...
    if (sen.entry_function.length() > 0) {
        ...
        E2LOGGER_error("No story_function defined in ntlm proxy auth plugin config");
        return -1;
    }
}
